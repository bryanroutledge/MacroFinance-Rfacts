...
exected inflation.  We can re-do this with real rates and it is a 
number <1% per year.
...
it is a hard number to estimtate.
...
The risk-free rate keeps you about on-par with incflation.  The stock market gets you $9,000+.  The ``stocks for the long run'' in one chart.
...
```{r}
Best.Worst(Flag.Year = 2020,N=12) %>% gt %>% fmt_number(columns = vars(R), decimals = 1, scale_by = 100, pattern="{x}%")
```
